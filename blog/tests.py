...
class BlogMethodTests(TestCase):
    ...
    def test_was_written_recently_with_article_written_over_seven_days_ago(self):
        ...
        """
        blog = Article(pub_date = timezone.now() - datetime.timedelta(days=10))
        self.assertEqual(blog.was_written_recently(), False)
    ...
    def test_was_written_recently_with_article_written_under_seven_days_ago(self):
        ...
        """
        blog = Article(pub_date = timezone.now() + datetime.timedelta(days=10))
        self.assertEqual(blog.was_written_recently(), True)
...
class BlogViewTests():
    ...
    def test_index_with_article_and_multiple_comments(self):
        ...
        self.assertQuerysetEqual(response.context['latest_article_list'], 
                                 ['<Article: title>'])
        self.assertQuerysetEqual(response.context['comment_list'], 
                                 ['<Comment: title 0>', '<Comment: title 1>', '<Comment: title 2>', '<Comment: title 3>', '<Comment: title 4>'])
    ...
    def test_index_with_multiple_articles_with_multiple_comments(self):
        ...
        self.assertQuerysetEqual(response.context['comment_list'], 
                                 ['<Comment: titleA 0>', '<Comment: titleA 1>', '<Comment: titleA 2>', '<Comment: titleA 3>', '<Comment: titleA 4>',
                                  '<Comment: titleB 0>', '<Comment: titleB 1>', '<Comment: titleB 2>', '<Comment: titleB 3>', '<Comment: titleB 4>',
                                  '<Comment: titleC 0>', '<Comment: titleC 1>', '<Comment: titleC 2>', '<Comment: titleC 3>', '<Comment: titleC 4>'])
    ...
    def test_comment_view_article_exists_but_comment_does_not(self):
        """
